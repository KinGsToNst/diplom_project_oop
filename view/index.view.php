...
<main id="js-page-content" role="main" class="page-content mt-3">
    ...
    <div class="row">
        <div class="col-xl-12">
            <?php if(QueryBuilder::is_admin(QueryBuilder::get_authentificated_user()) ):?>
                ...
                <div id="c_1" class="card border shadow-0 mb-g shadow-sm-hover" data-filter-tags="oliver kopyov">
                        ...
                        <div class="d-flex flex-row align-items-center">
                            ...
                            <div class="info-card-text flex-1">
                                <a href="javascript:void(0);" class="fs-xl text-truncate text-truncate-lg text-info" data-toggle="dropdown" aria-expanded="false">
                                    <?php if (!empty($user["user_name"])) { echo $user["user_name"];} else { echo "noname";}?>



                                    <?php if(QueryBuilder::is_admin(QueryBuilder::get_authentificated_user()) || QueryBuilder::is_equal($user,QueryBuilder::get_authentificated_user())):?>
                    ...
                    <div class="card-body p-0 collapse show">
                        <div class="p-3">
                            ...
                            <a href="tel:<?php if (!empty($user["phone"])) { echo $user["phone"];} else { echo "no phone";}?>" class="mt-1 d-block fs-sm fw-400 text-dark">
                                <i class="fas fa-mobile-alt text-muted mr-2"></i> <?php if (!empty($user["phone"])) { echo $user["phone"];} else { echo "нет телефона";}?></a>
                            <a href="mailto:<?=$user["email"]?>" class="mt-1 d-block fs-sm fw-400 text-dark">
                                <i class="fas fa-mouse-pointer text-muted mr-2"></i> <?=$user["email"]?></a>
                            <address class="fs-sm fw-400 mt-4 text-muted">


                                <i class="fas fa-map-pin mr-2"></i> <?php if (!empty($user["address"])) { echo $user["address"];} else { echo "нет адреса";}?></address>
                            <div class="d-flex flex-row">

                                <a href="<?php if (!empty($user["vk"])) { echo $user["vk"];} else { echo "javascript:void(0);";}?>" class="mr-2 fs-xxl" style="color:#4680C2">
                                    <i class="fab fa-vk"></i>
                                </a>
                                <a href=" <?php if (!empty($user["telegram"])) { echo $user["telegram"];} else { echo "javascript:void(0);";}?>
                                    " class="mr-2 fs-xxl" style="color:#38A1F3">
                                    <i class="fab fa-telegram"></i>
                                </a>

                                <a href="<?php if (!empty($user["instagram"])) { echo $user["instagram"];} else { echo "javascript:void(0);";}?>
